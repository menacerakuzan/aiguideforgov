...
interface Achievement {
  ...
  earned: (ctx: { progress?: MyProgressResponse }) => boolean;
}
...
export default function DashboardPage() {
  const { data: progress, error } = useQuery({
    ...
    queryFn: () => api.get<MyProgressResponse>('/api/progress'),
    retry: false,
  });

  const noActiveCourse = error instanceof ApiFetchError && error.status === 404;
  ...
  const { data: currentModule } = useQuery({
    enabled: !!current,
    queryKey: ['modules', current?.slug],
    queryFn: () => api.get<{ module: Module }>(`/api/modules/${current!.slug}`),
  });

  const todayTasks = (currentModule?.module.lessons ?? []).filter((l) => !l.completed).slice(0, 3);
...
function AchievementsDialog({ progress }: { progress?: MyProgressResponse }) {
  return (
    <Dialog>
