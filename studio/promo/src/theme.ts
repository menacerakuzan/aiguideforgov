...
/**
 * Шрифти вантажимо ЯВНО, а не покладаємось на назву в `font-family`.
 *
 * Remotion малює кадри у власному Chromium, де жодного Comfortaa в системі
 * немає: рядок `'"Comfortaa", …, sans-serif'` мовчки зводиться до системного
 * гротеску, і весь ролик виходить не тим шрифтом, що платформа. Помітно це
 * тільки на готовому кадрі, тому й записано тут.
 *
 * Береться лише те, що справді в кадрі: кирилиця з латиницею й потрібні
 * накреслення. Інакше Remotion тягне всі підмножини всіх ваг на КОЖЕН кадр
 * рендера — сотні зайвих запитів.
 */
const SUBSETS: ('cyrillic' | 'latin')[] = ['cyrillic', 'latin'];

const { fontFamily: comfortaa } = loadComfortaa('normal', {
  subsets: SUBSETS,
  weights: ['600', '700'],
